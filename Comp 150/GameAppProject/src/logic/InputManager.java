...
public class InputManager {
	...
	//public int choice = getActionChoice2();
	...
	public boolean validateUserChoice(int choice, int n) {
		...
		while(inInputValid == false){
			Integer testValue = willItParse(Integer.toString(choice));
			if(testValue == null){
				...
				if(is012(choice)){
					inInputValid = true;	inInputValid = true;
	...
	public int getActionChoice2(){
		int choice = -1;
		boolean validChoice = false;
		while (!validChoice){
			promptForChoice("Choose Wisely",myFate.getValues());
			choice = getInputInt("You choose to : ");
		}
		int n = myFate.size();
		validChoice = validateUserChoice(choice, n);
		switch (myFate.values()[choice]){
			case SPARE_HIM:
				setHistory("You Spare Him");
				break;
			case KILL_HIM:
				setHistory("You Put an end to his misery");
				break;
		}
	return choice;
	};
